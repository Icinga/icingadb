...
CREATE TABLE dependency_node (
  ...
  CONSTRAINT ck_dependency_node_either_checkable_or_redundancy_group_id CHECK ((
    CASE WHEN host_id IS NOT NULL AND service_id IS NULL THEN 1 ELSE 0 END +
    CASE WHEN service_id IS NOT NULL AND host_id IS NOT NULL THEN 1 ELSE 0 END +
    CASE WHEN redundancy_group_id IS NOT NULL THEN 1 ELSE 0 END
  ) = 1)
);
