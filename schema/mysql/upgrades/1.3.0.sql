...
-- Insert an sla lifecycle create_time entry for all existing hosts with the LEAST timestamp found in either
...
-- Insert an sla lifecycle deleted entry for all not existing hosts with the GREATEST timestamp
...
-- Insert an sla lifecycle create_time entry for all existing services with the LEAST timestamp found in either
-- the sla_history_state or sla_history_downtime table, otherwise fallback to the current Unix timestamp.
