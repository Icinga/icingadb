		...
		t.Run("State", func(t *testing.T) {
			...
			for index := range 3 {
				...
				t.Run("Verify-"+host.Name, func(t *testing.T) {
					...
					processCheckResult(t, client, host.Name, 2)
					...
					eventually.Assert(t, func(t require.TestingT) {
						...
						err = db.Get(&row, `SELECT is_acknowledged, is_sticky_acknowledgement, state_type
							FROM host_state
							  ...
							  INNER JOIN host ON host.host_id = host.id
							WHERE host.name = ?`, host.Name,
						)
						require.NoError(t, err, "querying service state should not fail")
						require.Equal(t, types.Bool{Bool: index > 0, Valid: true}, row.IsAcknowledged, "service should be acknowledged")
