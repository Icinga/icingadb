...
Using the `systemd-journald` logging output results in sending structured log entries to journald,
including fields with additional information that would be included in the `console` logging output mode.

When examining the journal with `journalctl`, these fields are hidden by default.
Setting the `--output` to another
[output option](https://www.freedesktop.org/software/systemd/man/latest/journalctl.html#Output%20Options)
will include them, e.g., `journalctl --output verbose` or `journalctl --output json`.
