...
func (r *RuntimeUpdates) Sync(
	...
	if len(streams) > 1 {
		return errors.New("too many Redis streams provided, expected at most 1")
	}
	...
	for _, factoryFunc := range factoryFuncs {
		...
		if opts.upsertFn != nil {
			...
			updateMessages, upsertEntities, deleteIds := prepareForSync(s, nil)
			if xReads[1] == nil {
	...
	// allowed to send XDel commands after all of them have processed the messages and sent the IDs to be deleted
	// to the xRedisMessageAcks channels. This way, they can move forward independently without waiting for the
	// others to finish processing the same batch of messages.
...
func (r *RuntimeUpdates) xRead(
	...
	return func() error {
		defer func() {
			defer close(acknowledgementOutCh)
