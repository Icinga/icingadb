...
var (
	// timeout for HA heartbeat.
	timeout = 60 * time.Second
	// timeoutGracePeriod is an additional grace period for the timeout to declare the other node timed out.
	timeoutGracePeriod = 3 * time.Second
)
	...
	err := retry.WithBackoff(
		...
		func(ctx context.Context) error {
			...
			case errQuery == nil:
				...
				if instance.Heartbeat.Time().Before(time.Now().Add(-1 * (timeout + timeoutGracePeriod))) {
...
func (h *HA) removeOldInstances(s *icingaredisv1.IcingaStatus, envId types.Binary) {
	...
	case <-h.ctx.Done():
		return
	case <-time.After(timeout + timeoutGracePeriod):
		query := h.db.Rebind("DELETE FROM icingadb_instance " +
			"WHERE id <> ? AND environment_id = ? AND endpoint_id = ? AND heartbeat < ?")
		heartbeat := types.UnixMilli(time.Now().Add(-1 * (timeout + timeoutGracePeriod)))
