...
func Test_relations_complete(t *testing.T) {
	t.Run("demo provider", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			rel.Host)
		assert.Equal(t, []string{"object.type", "host.name", "host.display_name"}, rel.completeRelations)
		...
		assert.Equal(t,
			...
			rel.Host)
		assert.Equal(t, []string{"object.type", "host.name", "host.display_name", "host.vars"}, rel.completeRelations)
