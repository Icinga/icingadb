...
// fetchObjectFromSql executes a query for one ID parameter and returns the output in the output argument pointer.
...
func (client *Client) fetchMultiCustomVarsFromSql(
	...
	// While host and servicegroups could have customvars, this is currently explicitly not supported in Notifications.
	panicForInvalidType([]string{"host", "service"}, typ)
...
type relations struct {
	// hostId and serviceId, where latter can be nil for hosts.
	hostId    types.Binary
	...
	for _, selector := range segments[0].Selectors() {
		...
		if len(childSegments) == 0 {
			...
			if rootSelector == `"host"` {
				...
				// services - or all instances of the object types - is being fetched. Due to the missing field, this
		...
		updateCompleteRelations := func(prefix string, isGroup bool) {
			...
			if fetchObject {
				// Both name and display_name are fetched together via Redis.
		...
		case `"services"`:
			if rel.serviceId != nil {
				...
				// IDs are re-fetched every time, as caching those are another layer of complexity for coping.
